...
> 로그인 후 첫 화면으로, 전체 환자 목록을 실시간으로 조회합니다.

![환자 목록 대시보드](docs/images/patient_list.png)

| 기능 | 설명 |
|------|------|
| 환자 목록 테이블 | 환자번호 · 이름 · 나이 · 성별 · 혈액형 · 최근검사일 · 진료 상태 표시 |
| 상태 필터 | 전체 / 진료 완료 / 진료 대기 드롭다운 필터 |
| 환자 검색 | 이름 또는 환자번호로 실시간 검색 |
| 상세 이동 | 행 더블클릭 시 해당 환자의 상세 분석 페이지로 이동 |
| LIVE 배지 | 신규 접수 환자가 실시간으로 반영됨을 표시 |

---

### 환자 상세 — 진단 분석 리포트

> 환자를 선택하면 AI 진단 결과와 이전 리포트 이력을 한 화면에서 확인합니다.

![환자 상세 화면](docs/images/patient_detail.png)

| 영역 | 설명 |
|------|------|
| 좌측 사이드바 | 6개 질환 모듈(신부전·뇌종양·안과·척추·피부·대장암) 탭 전환 |
| 환자 정보 헤더 | 나이·성별·혈액형·최근 검사일·담당의 요약 표시 |
| 진단 분석 리포트 | 모듈별 AI 진단 결과, 신뢰도, LLM 소견서 리스트로 표시 |
| 누적 리포트 수 | 해당 환자의 전체 진단 건수 뱃지로 표시 |
| 신규 진단 버튼 | 선택한 모듈에서 새 AI 진단 즉시 실행 |
